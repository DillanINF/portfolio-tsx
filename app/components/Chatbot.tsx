...
export default function Chatbot() {
  ...
  const handleSendMessage = async () => {
    ...
    setTimeout(() => {
      ...
      // 1. Check for yapping (too many messages) - prioritas tertinggi
      ...
      // 2. Check for disbelief reactions
      else if (matchesWithTypo(inputValue, ['gw kaya', 'gw tajir', 'gw sultan', 'gw jenius', 'gw pintar banget', 'gw expert', 'gw pro', 'gw hebat', 'gw master'], 0.7)) {
        botResponse = {
          id: (Date.now() + 1).toString(),
          text: "Apa? Serius lu? 🤨",
          isUser: false,
      ...
      // 3. Check if message should have specific image
      else {
        const imageResponse = getImageResponse(inputValue);
        
        if (imageResponse) {
          // Has specific image response
          botResponse = {
            id: (Date.now() + 1).toString(),
          ...
          const textResponse = getResponse(inputValue);
          const isDefaultResponse = responses.default.includes(textResponse);
          
          if (isDefaultResponse) {
            // Chatbot confused - use monyet.jpg
            botResponse = {
              id: (Date.now() + 1).toString(),
              text: textResponse,
              isUser: false,
            ...
            // Normal response - randomly add image (30% chance)
            ...
            botResponse = {
              ...
              isUser: false,
              timestamp: new Date(),
              image: shouldAddImage ? getRandomCategoryImage(inputValue) : undefined
            };
          }
        }
      }
